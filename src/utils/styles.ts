...
export type letterSpacing = LetterSpacingThemeKeys | (string & Record<never, never>)
...
function toThemeValueIfNeeded<T>(propKey: string, value: T, theme?: AppTheme) {
  if (theme && theme.space && SPACE_KEYUS.has(propKey) && isSpaceThemeKeys(value, theme)) {
